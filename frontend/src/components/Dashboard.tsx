...
import { createCustomActivityName, getActivityConfig, STORAGE_KEYS } from '../constants';
...
export const Dashboard: React.FC = () => {
  ...
  const [tileOrder, setTileOrder] = useState<ActivityName[]>(
    isViewingOther ? [...ACTIVITY_NAMES] : loadTileOrder
  );
  const [tileSizes, setTileSizes] = useState<Record<ActivityName, TileSize>>(
    isViewingOther ? getDefaultTileSizes() : loadTileSizes
  );
  ...
  const fetchActivities = useCallback(async () => {
    ...
    // cacheActivities and loadCachedActivities are stable store functions
    // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        >
          ...
          >
            ...
            {!isPrivateAccount && (
              <button
                onClick={() =>
                  targetUsername && navigate(APP_ROUTES.USER_ANALYTICS(targetUsername))
                }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flexShrink: 0 }}>
            <button
              ...
              style={{
                ...
                padding: '8px 16px',
                background:
                  hiddenTiles.length > 0 ? 'rgba(251, 191, 36, 0.15)' : 'rgba(0, 149, 246, 0.1)',
