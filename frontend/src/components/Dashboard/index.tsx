...
export const Dashboard: React.FC = () => {
  ...
  // Activity data
  const { activities, setActivities, activityNotes, setActivityNotes, loading, fetchActivities } =
    useActivityData({
      targetUsername,
      currentDate,
      isReadOnly,
  ...
  // Drag and drop
  const { activeDragId, sensors, handleDragStart, handleDragEnd } = useDragAndDrop({
    setTileOrder,
  });
  ...
  const handleAvatarClick = () => {
    ...
    if (hasStories) {
      const sortedPhotos = [...targetUserPhotos].sort(
  ...
  return (
    <PullToRefreshWrapper onRefresh={handlePullToRefresh} disabled={isPullToRefreshDisabled}>
      <div className="container" style={{ paddingBottom: '2rem' }}>
        ...
        {loading || configLoading ? (
          ...
          <PrivateAccountView targetUsername={targetUsername || ''} badges={privateAccountBadges} />
        ) : (
          <TileGrid
